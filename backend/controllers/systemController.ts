...
import { getAuditLogs, AUDIT_ACTIONS } from '../middleware/audit';
...
const formatLogMessage = (log: AuditLogEntry): string => {
...
const getSystemLogs = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const limit = Math.min(200, Math.max(1, parseInt(req.query.limit as string, 10) || 50));
    const logs = await getAuditLogs({ limit });

    const formattedLogs = (logs as AuditLogEntry[]).map((log, idx) => {
      let type = 'INFO';
      if (log.action) {
...
const getSystemMetrics = async (req: Request, res: Response): Promise<void> => {
  try {
